...
idata = load_iris('./data/iris_dataset/iris_sklearn.csv')

for k, v in idata.items():
    print k, v[:5]

ikeys = ['sepal_length', 'sepal_width', 'petal_length', 'petal_length']

data1 = map(lambda k: idata[k][idata['class'] == 0],
        ikeys)

data2 = map(lambda k: idata[k][idata['class'] == 1],
        ikeys)

data3 = map(lambda k: idata[k][idata['class'] == 2],
        ikeys)

class1_sample = np.vstack(data1)
class2_sample = np.vstack(data2)
class3_sample = np.vstack(data3)
...
ax[1,0].scatter(xrange(4), mlab_pca.fracs)
plt.legend()


# sklearn
stdScaler = StandardScaler()
sklearn_pca = sklearnPCA(n_components=4, svd_solver='randomized')
